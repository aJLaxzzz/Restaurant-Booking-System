...
func (a *Handlers) handleReservationsList(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		_ = rows.Scan(&id, &uid, &fn, &phone, &tid, &tnum, &st, &end, &guests, &status, &createdBy)
		out = append(out, map[string]any{
			"id": id.String(), "user_id": uid.String(), "client_name": fn, "phone": phone,
			"table_id": tid.String(), "table_number": tnum, "start_time": st, "end_time": end,
			"guest_count": guests, "status": status, "created_by": createdBy,
		})
	}
	a.json(w, http.StatusOK, out)
}
...
func (a *Handlers) handleReservationCreate(w http.ResponseWriter, r *http.Request) {
	...
	var existingPay uuid.UUID
	var existingRes uuid.UUID
	err := a.Pool.QueryRow(r.Context(), `
		SELECT reservation_id, id FROM payments WHERE idempotency_key=$1`, body.IdempotencyKey).
		Scan(&existingRes, &existingPay)
	if err == nil {
		a.json(w, http.StatusOK, map[string]any{
			"reservation_id": existingRes.String(),
			"payment_id":     existingPay.String(),
			"checkout_url":   "/pay/" + existingPay.String(),
			"duplicate":      true,
		})
		return
	}
	...
	slotHours := a.getSettingInt(r.Context(), "default_slot_duration_hours", 2)
	...
	err = a.Pool.QueryRow(r.Context(), `
		SELECT t.capacity, t.status, t.hall_id FROM tables t WHERE t.id=$1`, body.TableID).
		Scan(&cap, &status, &hallID)
	if err != nil {
		if err == pgx.ErrNoRows {
	...
	depositPct := a.getSettingInt(r.Context(), "deposit_percent", 30)
	avgKop := a.getSettingInt(r.Context(), "avg_check_kopecks", 150000)
	...
	err = tx.QueryRow(r.Context(), `
		...
		VALUES ($1,$2,$3,$4,$5,'pending_payment',$6,'client')
		RETURNING id`,
		body.TableID, u.ID, body.StartTime, endTime, body.GuestCount, nullStr(body.Comment)).Scan(&resID)
	if err != nil {
		_ = a.RDB.Del(r.Context(), key).Err()
	...
	err = tx.QueryRow(r.Context(), `
		...
		VALUES ($1,$2,'pending',$3) RETURNING id`, resID, amountKop, body.IdempotencyKey).Scan(&payID)
	if err != nil {
		_ = a.RDB.Del(r.Context(), key).Err()
