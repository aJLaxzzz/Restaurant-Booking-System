...
1) Папка Auth → запрос Login (email/password демо из docs/DEMO_USERS.md).
   В Tests токен записывается в переменные; у каждого запроса в коллекции есть
   pm.test(...) (общие: не 5xx, время ответа; плюс именованная проверка), иначе
   Runner/Newman могли бы показывать «No tests found» для запросов без своих тестов.
